...
for m in range(M):
	# print(m, 'of', M, 'angles')
	index = 0
	a_theta = (np.exp(1j * (2 * np.pi / lamb) * (np.array([np.cos(theta_search[m]), np.sin(theta_search[m])]) @ r_array)))
	# print(np.shape(a_theta))

	for q in range(Q):
		b_tau = np.exp(-1j * 2 * np.pi * lf_vector * tau_search[q] * delta_f)
		# print(np.shape(b_tau))
		U = np.kron(a_theta, b_tau)
		print(np.shape(U))
		# peform barlett with Uqq
		numerator = U.conj().T @ R_xx @ U
		denominator = U.conj().T @ U 

		P_barlett[index,q] = numerator / denominator
		index = index + 1
